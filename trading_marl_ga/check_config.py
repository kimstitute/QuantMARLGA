...
print(f"  Buffer/Batch 비율:         {config.BUFFER_CAPACITY/config.BATCH_SIZE:.1f}x")
...
print(f"  Learning Rate (Encoder):   {config.ENCODER_LEARNING_RATE}")
...
print(f"  자식 변이 확률:            {config.CHILD_MUTATION_PROB*100:.0f}% (90% 자식 mutate)")
print(f"  파라미터 변이 확률:        {config.PARAM_MUTATION_PROB*100:.0f}% (각 파라미터)")
print(f"  상대적 노이즈 비율:        {config.MUTATION_SCALE_RATIO*100:.1f}% (가중치 대비)")
print(f"  실제 변이율:               {config.CHILD_MUTATION_PROB * config.PARAM_MUTATION_PROB * 100:.1f}% (sparse mutation)")
...
print(f"  세대당 샘플 수:            {config.RL_UPDATES * config.BATCH_SIZE:,} (updates × batch_size)")
print(f"  예상 Epochs (세대 4):      ~{(config.RL_UPDATES * config.BATCH_SIZE) / (660 * 4):.1f} (buffer ~2640개 기준)")
