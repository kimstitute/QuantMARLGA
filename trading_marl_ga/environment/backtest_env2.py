...
if config.DATA_SOURCE == "real":
    from data.market_data_manager2 import MarketDataManager2
...
class BacktestEnv2:
    ...
    4. 학습 시 사용한 종목을 지정하여 테스트 가능 (tickers 파라미터)
    
    데이터 소스:
    - config.DATA_SOURCE = "real": MarketDataManager2로 실제 데이터
    ...
    def __init__(self, n_days=None, start_date=None, end_date=None, data_manager=None, tickers=None):
        ...
        Args:
            ...
            data_manager (MarketDataManager2): 이미 초기화된 데이터 관리자 (재사용)
            tickers (list): 사용할 종목 코드 리스트 (학습 종목과 동일하게 테스트 시 필요)
        """
        self.n_stocks = config.N_STOCKS
        self.initial_capital = config.INITIAL_CAPITAL
        
        print(f"\n{'='*60}")
        print(f"BacktestEnv2 초기화")
        print(f"{'='*60}")
        ...
        if config.DATA_SOURCE == "real":
            ...
            if data_manager is not None:
                print("[OK] 기존 MarketDataManager2 재사용")
                self.data_manager = data_manager
            else:
                print("[OK] 새 MarketDataManager2 생성")
                self.data_manager = MarketDataManager2(cache_dir=config.CACHE_DIR)
